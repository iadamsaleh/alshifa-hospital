...
export default function PharmacyInvoicesPage() {
  ...
  useEffect(() => {
    ...
    authFetch(`/pharmacy/invoices?${params.toString()}`)
      .then((res) => {
        if (!res.ok) throw new Error("Failed to load pharmacy invoices");
        return res.json();
      })
      .then(setResult)
  ...
  return (
    <div className="p-8">
      <header className="mb-6">
        <h1 className="text-2xl font-semibold text-slate-800">Pharmacy Invoices</h1>
        <p className="text-slate-500 text-sm">Full history of pharmacy sales.</p>
      </header>
      ...
      <section className="bg-white rounded-lg shadow overflow-hidden">
        ...
        {loading ? (
          ...
          <p className="text-slate-500 text-sm p-4">No pharmacy invoices found.</p>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y divide-slate-100">
              {result.data.map((inv) => (
                <tr
                  key={inv.id}
                  onClick={() => navigate(`/pharmacy/invoices/${inv.id}`)}
                ...
                >
                  ...
                  <td className="px-4 py-2 text-slate-800">
                    {inv.patient ? inv.patient.name : <span className="text-slate-400 italic">Walk-in</span>}
                  </td>
                  <td className="px-4 py-2 text-slate-600">{inv.items.length}</td>
                  <td className="px-4 py-2 text-slate-600">PKR {inv.totalAmount}</td>
                  <td className="px-4 py-2">
                    <span
                      className={`text-[11px] font-semibold px-2 py-0.5 rounded-full ${STATUS_STYLES[inv.status]}`}
                    >
                      {inv.status}
                    </span>
                  </td>
