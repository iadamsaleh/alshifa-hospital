...
export default function PharmacyInvoiceViewPage() {
  const { id } = useParams();
  ...
  function load() {
    setLoading(true);
    return authFetch(`/pharmacy/invoices/${id}`)
      .then((res) => {
  ...
  async function handleMarkPaid() {
    setMarking(true);
    try {
      const res = await authFetch(`/pharmacy/invoices/${id}/pay`, { method: "PATCH" });
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6 print:hidden">
        <Link to="/pharmacy/invoices" className="text-sm text-blue-950 hover:underline">
          ← Back to Pharmacy Invoices
        </Link>
      ...
      <div className="print-area max-w-2xl mx-auto bg-white rounded-lg shadow p-8">
        <div className="flex items-start justify-between border-b border-slate-200 pb-4 mb-4">
          ...
          <div className="text-right">
            <p className="text-sm font-semibold text-slate-800">Pharmacy Invoice</p>
        ...
        <div className="grid grid-cols-2 gap-y-1 text-sm mb-6">
          {patient ? (
            ...
            <p className="text-slate-500 italic col-span-2 text-center">Walk-in Purchase (no patient attached)</p>
          )}
        </div>
        ...
        <table className="w-full text-sm mb-4">
          <thead className="text-left text-slate-500 border-b border-slate-200">
            <tr>
              ...
              <th className="py-2 font-medium text-right">Unit Price (PKR)</th>
              <th className="py-2 font-medium text-right">Subtotal (PKR)</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {invoice.items.map((it) => (
              <tr key={it.itemId}>
                <td className="py-2 text-slate-800">{it.itemName}</td>
                <td className="py-2 text-right text-slate-600">
                  {it.quantity} {it.unit}
                </td>
                <td className="py-2 text-right text-slate-800">{it.price}</td>
                <td className="py-2 text-right text-slate-800">{it.price * it.quantity}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <div className="flex justify-end border-t border-slate-200 pt-4">
          <p className="text-lg font-semibold text-blue-950">Grand Total: PKR {invoice.totalAmount}</p>
        </div>
      </div>
    </div>
  );
