...
export default function PrescriptionViewPage() {
  ...
  async function handleSendWhatsApp() {
    ...
    try {
      if (window.electronAPI?.exportPdf) {
        await window.electronAPI.exportPdf(`prescription-${prescription.patient.patientId}.pdf`);
      }
      await openWhatsApp(whatsAppPrescriptionUrl(prescription.patient.phone));
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-6 print:hidden max-w-2xl mx-auto">
        <Link to={`/patients/${prescription.patient.id}`} className="text-sm text-blue-950 hover:underline">
