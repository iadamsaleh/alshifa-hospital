export default function OpdTokenSlip({ result, onReset }) {
  const { patient, doctor, visit } = result;
  const visitAt = new Date(visit.visitDate);
    ...
    <div className="p-8">
      <div className="max-w-md mx-auto">
        <div className="print-area bg-white rounded-lg shadow p-8 text-center border border-slate-200">
          ...
          <p className="text-xs text-slate-500 mb-6">OPD Visit Token</p>

          <p className="text-5xl font-bold text-blue-950 mb-6">{visit.tokenLabel}</p>
          ...
          <dl className="text-left text-sm grid grid-cols-2 gap-y-2 border-t border-dashed border-slate-300 pt-4">
            ...
            <dd className="text-slate-800 font-medium text-right">{visitAt.toLocaleDateString()}</dd>

            <dt className="text-slate-500">Time</dt>
            <dd className="text-slate-800 font-medium text-right">{visitAt.toLocaleTimeString()}</dd>
          </dl>
        </div>
